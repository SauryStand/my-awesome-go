...
import (
	...
	"kitfw/src/kitfw/sg/pb"

	protocol "kitfw/src/kitfw/sg/protocol"

	"context"

	"kitfw/src/kitfw/sg/define"
...
func concat(conn *grpc.ClientConn, tracer stdopentracing.Tracer, str1 string, str2 string) {
	...
	ctx := metadata.NewOutgoingContext(context.Background(),md)//NewContext(context.Background(), md)
