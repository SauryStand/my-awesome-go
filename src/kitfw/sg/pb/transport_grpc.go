...
func MakeGRPCServer(ctx context.Context, endpoint endpoint.Endpoint, tracer stdopentracing.Tracer, logger log.Logger) KitfwServer {
	...
	return &grpcServer{
		process: grpctransport.NewServer(
			...
			EncodeGRPCProcessResponse,//FromGRPCRequest(tracer, "Process", logger)
			append(options, grpctransport.ServerBefore(RequestMetaDataFunc), grpctransport.ServerBefore(opentracing.GRPCToContext(tracer, "Process", logger)))...,
		),
	}
}
